...
uint8_t set_val(frame_t* frame, frame_map_t* map_frame, uint8_t index, uint8_t* value, uint64_t limit);
    ...
    while(-1 == return_val) {
        ...
        switch (getc) {
            ...
            case 0: { // u8-add
                ...
                set_val(frame, map_frame, ret, &final, 1);
...
uint8_t* get_val(frame_t* frame, frame_map_t* map_frame, uint8_t index, map_t* map) { // gets value at index within a frame - handles accessing constants
    ...
    if(map_frame->const_len > index) {
        ...
        if(frame->values[index - map_frame->const_len] == NULL) { debug("Accessing undefined variable!"); }
        return_val = (uint8_t*) frame->values[index - map_frame->const_len];
    }
    return return_val;
}

uint8_t set_val(frame_t* frame, frame_map_t* map_frame, uint8_t index, uint8_t* value, uint64_t size) { // handles setting variables of a frame given an index and size IN BYTES
    ...
    if(frame->values[index - map_frame->const_len] == NULL) {
        frame->values[index - map_frame->const_len] = malloc(sizeof(uint8_t) * size);
    }
    memcpy(frame->values[index - map_frame->const_len], value, size);
